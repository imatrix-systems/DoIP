...
**Version:** 3.2.0
**Status:** Implementation Complete — End-to-end validated 2026-04-14 (FC-1 + DCU + Bastion web terminal)
...
| Asset | Owner | Storage | Scope |
|---|---|---|---|
| DCU private key | DCU | `id_ed25519` (0600, root) | DCU only |
| DCU public key | DCU | Bastion `authorized_keys` | Public |
| FC-1 private key | FC-1 | `id_ed25519` (0600, root) | FC-1 only |
| FC-1 public key | FC-1 | Bastion `authorized_keys` | Public |
| Bastion host key | Bastion | Pinned in firmware | Public |
| SSH CA private key | iMatrix Ops | HSM or offline | HSM only |
| SSH CA public key | All devices | `ssh_ca.pub` in firmware | Public |
| HMAC secret | FC-1 + DCU | `hmac_secret` (0600) | Paired devices |

All keys use ED25519 except the HMAC secret (HMAC-SHA256). Private keys are stored under `/etc/phonehome/` on the device.
...
| Asset | Owner | Storage | Sensitivity |
|---|---|---|---|
| SSH CA private key | iMatrix Ops | HSM or offline storage | **CRITICAL** — compromise allows bastion impersonation |
| SSH CA public key | All devices | `ssh_ca.pub` in firmware | **Public** — safe to embed, commit to repos |
| Bastion host key | Bastion | `ssh_host_ed25519_key` | Server-local |
| Bastion host cert | Bastion | `ssh_host_ed25519_key-cert.pub` | **Public** — signed by CA |

All assets use ED25519 algorithm.
...
- **SSH tunnel username is `tunnel`**, not `phonehome-<DCU_SERIAL>`. All devices (FC-1 and DCU) share the `tunnel` user account on the bastion. The bastion identifies each device by its SSH public key, which is registered with a comment tag (e.g., `device:0281183743:dcu`) in the shared `/home/tunnel/.ssh/authorized_keys` file. This simplifies bastion administration — no per-device user accounts or authorized_keys files are needed for tunnel establishment.
- **Design note (v3.2.0):** Earlier versions of this specification (v3.1.0 and prior) described a `phonehome-<DCU_SERIAL>` per-device username model. This was changed during implementation to the shared `tunnel` user model. The `phonehome-*` user pattern is reserved in the bastion sshd config (`Match User phonehome-*`) for future per-device access policies, but is not used by the current tunnel implementation.
...
def wait_for_dcu_tunnel(dcu_serial: str, timeout: int = 60) -> int:
    ...
    # when the tunnel user connects with the DCU's key and which port they bind.
...
2. Bastion logs show `tunnel` user connecting with DCU's key
